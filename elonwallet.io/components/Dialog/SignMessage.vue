    ...
    <div>
        <v-dialog v-model="dialog" persistent>
            <div class="p-4 flex flex-col w-2/6 bg-white m-auto">
                ...
                <p class="ml-2">{{ signingParams.message }}</p>
                <div class="flex-grow border-t-2 border-custom-light-blue opacity-30 my-3"></div>
...
import { extractMessageAndSigner } from '~/lib/WalletConnectUtils';
...
const signingParams = computed(() => {
    return extractMessageAndSigner(props.requestEvent.params.request.params);
})
...
const onApprove = async () => {
    try {
        const signature = await signMessage(signingParams.value.message, signingParams.value.address);
        const response = {
            id: props.requestEvent.id,
            jsonrpc: '2.0',
            result: signature
